...
import json
df = pd.read_csv('EducationLevel_SUDO_after cleaning.csv')
jdf = df.to_json(orient='records')
#records = json.loads(jdf)
with open('EducationLevel_SUDO_after cleaning.json', 'w') as f:
    f.write(jdf)
df['X'] = df.index.values

with open('EducationLevel_SUDO_after cleaning.json', 'r') as f:
    for line in f:
        records = json.loads(jdf)
